...
model_path = "checkpoints_v7/multi"
...
input_dataset = np.load("datasets/multisource/train_live.npy")
...
# This slowly reduces the LR to lock in the optimal weights without bouncing.
...
for epoch in range(num_epochs):
    ...
    for data in dataloader: 
        ...
        m_s_partial_GT = torch.tensor(GenerateSCMap_poly(s, 32, 32), dtype=torch.float32).to(device) / 255.0
        ...
        # --- PHASE V (REVISION C): MAGNITUDE-INVARIANT PUSH-PULL ---
        
        # 1. Native Global Average Pooling
        # We allow the length to naturally vary based on spatial diversity. 
        # No artificial F.normalize() blowing up the gradients!
        z_live_gap = live_feature.mean(dim=[2, 3])
        z_spoof_gap = partial_spoof_z.mean(dim=[2, 3])
        
        # 2. Dynamic Batch Center
        c = z_live_gap.mean(dim=0, keepdim=True).detach()
        
        # 3. PULL: Cosine Distance (The fix!)
        # 1.0 - sim mathematically bounds the loss between 0 and 2. 
        # This aligns the vectors perfectly without gradient explosions.
        sim_pull = torch.nn.functional.cosine_similarity(z_live_gap, c.expand_as(z_live_gap), dim=1)
        L_pull = torch.mean(1.0 - sim_pull)
        
        # 4. PUSH: Cosine Margin
        sim_push = torch.nn.functional.cosine_similarity(z_spoof_gap, c.expand_as(z_spoof_gap), dim=1)
        L_push = torch.mean(torch.nn.functional.relu(sim_push - 0.05))
        
        lambda_pull = 0.05 
        lambda_push = 0.05

        # 5. ASYMMETRIC LOSS: The 2.5 Scalpel remains
        loss_ex_g = 2.5 * map_loss_l + 1.0 * partial_m_loss + lambda_pull * L_pull + lambda_push * L_push
        ...
        m_s_partial_GT = torch.tensor(GenerateSCMap_poly(s), dtype=torch.float32).to(device) / 255.0
        m_s_partial_GT = rearrange(m_s_partial_GT, 's h w -> s 1 h w')
        ...
        loss_g = 1.0 * partial_m_loss + cos_loss_a_partial
        ...
        if step % savestep == 0:
            ...
            savecount += 1
    # Step the learning rate down at the end of each epoch
    scheduler.step()
