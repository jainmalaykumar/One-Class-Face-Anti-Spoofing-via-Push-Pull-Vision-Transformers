...
model_path = "checkpoints_v7/casia"

test_live_data = np.load("datasets/casia_mfsd/test_live.npy")
test_spoof_data = np.load("datasets/casia_mfsd/test_spoof.npy")
...
labels0 = np.zeros(test_live_data.shape[0]) # Live should be 0 (Normal)
labels1 = np.ones(test_spoof_data.shape[0]) # Spoof should be 1 (Anomaly)

test_images = np.vstack([test_live_data, test_spoof_data]) 
test_labels = np.hstack([labels0, labels1]) # Match the stack order
